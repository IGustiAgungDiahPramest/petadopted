...
class DataService {
  ...
  static Future<List<CustomerServis>> fetchCustomerService() async {
    final response = await http.get(Uri.parse(Endpoints.customer)); 
    if (response.statusCode == 200) {
      final data = jsonDecode(response.body) as Map<String, dynamic>;
      return (data['datas'] as List<dynamic>) 
          .map((item) => CustomerServis.fromJson(item as Map<String, dynamic>))
          .toList();
    } else {
      // Handle error
      throw Exception('Failed to load data');
  ...
  static Future<void> deleteCustomerService(String id) async { 
    await http.delete(Uri.parse('${Endpoints.customer}/$id'),
